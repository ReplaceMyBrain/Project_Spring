<%@page import="com.springlec.base.dto.ItemInfoDto" %>
...
@media ( max-width : 1000px) {
	nav, article {
		...
		height: auto;
			...
			//Empty Check
			function ItemUpdate(){
				var form = document.ItemInfoContent
		...
		<article>
			<section>
				<form action="ItemUpdate" method="post"
					enctype="multipart/form-data" name="ItemInfoContent">
					<input type="hidden" value="${i_num}" name="I_NUM">
					<%-- ${ItemInfoContent.I_NUM} --%>
					<br>
					...
					<table class="table table-bordered"
						...
						<tr>
							<th>글 제목 </th>
							<td><input type="text" name="I_TITLE" placeholder="글 제목" style="margin: 0px; width: 1160px;" class="form-control" value="${i_title}"></td>
							<%-- "${ItemInfoContent.I_TITLE}" --%>
						</tr>
						<tr>
							<th>카테고리 </th>
						...
						<tr>
							<th>사진</th>
							<td><c:set var="requestFilePath" value="${i_image }" />
							<%-- ${ItemInfoContent.I_IMAGE } --%>
								<c:choose>
									<c:when test="${!empty requestFilePath}">
										<a href="${i_image }" download>${fileName }</a>
									</c:when>
									<c:otherwise>
										<a>첨부파일 없음</a>
									</c:otherwise>
								</c:choose> <!-- 이미지 수정을 위해서 기존 파일 경로도 전송해야하므로 히든아이템으로 유지한다. --> <input
								type="hidden" name="oldFilePath" value="${i_image }">
								<input type="file" name="I_IMAGE"></td>
						</tr>
						<tr>
							<th>첨부파일 미리보기</th>
							<td><img width="500" src="${i_image }"/></td>
						</tr>
						<tr>
						<th>내용</th>
							<td><textarea style="margin: 0px; width: 1160px; height: 370px;" name="I_CONTENT"
									class="form-control" placeholder="내용을 입력하세요.">${i_content}</textarea></td>
									<%-- ${ItemInfoContent.I_CONTENT} --%>
						</tr>
					</table>
						<div style="text-align: center;">
