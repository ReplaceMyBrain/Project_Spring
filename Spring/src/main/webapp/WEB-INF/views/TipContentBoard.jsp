...
<script type="text/javascript">
			//Empty Check
			function UpdateTip(){
				var form = document.TipContentBoard
...
<body>
	...
	<div class="container">
		...
		<article>
			<section>
				<form action="TipContentBoard" method="post" name="TipContentBoard"
					enctype="multipart/form-data">
					<input type="hidden" value="${t_num}" name="T_NUM">
					<%-- ${TipUpdateBoard.T_NUM} --%>
					...
					<table class="table table-bordered"
						...
						<tr>
							...
							<td><input type="text" name="T_TITLE" placeholder="글 제목"
								class="form-control"  style="margin: 0px; width: 1160px;" value="${t_title}"></td>
								<%-- ${TipUpdateBoard.T_TITLE} --%>
						</tr>
						<tr>
							<th>카테고리 </th>
						...
						<tr>
							<th>사진</th>
							<td><c:set var="requestFilePath" value="${t_image }" />
								<c:choose>
									<c:when test="${!empty requestFilePath}">
										<a href="${t_image }" download>${fileName }</a>
									</c:when>
									<c:otherwise>
										<a>첨부파일 없음</a>
									</c:otherwise>
								</c:choose> <!-- 이미지 수정을 위해서 기존 파일 경로도 전송해야하므로 히든아이템으로 유지한다. --> <input
								type="hidden" name="oldFilePath" value="${t_image }">
								<%-- ${TipUpdateBoard.T_IMAGE } --%>
								<input type="file" name="T_IMAGE"></td>
						</tr>
						<tr>
							<th>첨부파일 미리보기</th>
							<td><img width="500" src="${t_image }" /></td>
						</tr>
						<tr>
						<th>내용</th>
							<td><textarea style="margin: 0px; width: 1160px; height: 386px;" name="T_CONTENT"
									class="form-control" placeholder="내용을 입력하세요.">${t_content}</textarea></td>
									<%-- ${TipUpdateBoard.T_CONTENT} --%>
						</tr>
					</table>
					<div style="text-align: center;">
