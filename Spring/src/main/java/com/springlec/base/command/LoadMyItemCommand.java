...
public class LoadMyItemCommand implements Command {
	...
	public void execute(Model model, HttpSession httpSession, SqlSession sqlSession) {
		...
		model.addAttribute("myList", listDao.myItemListDao(currentUser, offset, numOfTuplesPerPage));
		model.addAttribute("myprofile", dao.loadProfileDao(currentUser));
		model.addAttribute("count", Integer.toString(countedTuple));
		model.addAttribute("do", "profileOnlyTool");
		
//		httpSession.setAttribute("do", "profile.do");
